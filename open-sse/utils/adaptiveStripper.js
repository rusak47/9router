...
export function extractRejectedFieldNamesFromError(responseBody) {
  if (typeof responseBody === "string") {
    ...
    console.debug("[FIELDSTRIP-PARSER] string input, fields:", fields);
    return fields;
  }
  if (typeof responseBody === "object" && responseBody !== null) {
    ...
    if (Array.isArray(details)) {
      ...
      console.debug("[FIELDSTRIP-PARSER] Mistral 422 detail, fields:", fields);
      return fields;
    }
    if (responseBody.error?.message) {
      const fields = extractRejectedFieldNames(responseBody.error.message);
      console.debug("[FIELDSTRIP-PARSER] error.message, fields:", fields);
      return fields;
    }
  }
  console.debug("[FIELDSTRIP-PARSER] no parseable error format");
  return [];
}
